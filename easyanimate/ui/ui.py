...
class EasyAnimateController:
    def __init__(self, low_gpu_memory_mode, weight_dtype):
        ...
        self.diffusion_transformer_dir = os.path.join(
            self.basedir, "models", "Diffusion_Transformer"
        )
        self.motion_module_dir = os.path.join(self.basedir, "models", "Motion_Module")
        self.personalized_model_dir = os.path.join(
            self.basedir, "models", "Personalized_Model"
        )
    ...
    def refresh_diffusion_transformer(self):
        self.diffusion_transformer_list = sorted(
            glob(os.path.join(self.diffusion_transformer_dir, "*/"))
        )

    def refresh_motion_module(self):
        motion_module_list = sorted(
            glob(os.path.join(self.motion_module_dir, "*.safetensors"))
        )
        self.motion_module_list = [os.path.basename(p) for p in motion_module_list]

    def refresh_personalized_model(self):
        personalized_model_list = sorted(
    ...
    def update_edition(self, edition):
        ...
        if edition == "v1":
            self.inference_config = OmegaConf.load(
                os.path.join(self.config_dir, "easyanimate_video_motion_module_v1.yaml")
            )
            return (
                gr.update(),
                ...
                gr.update(value=512, minimum=384, maximum=704, step=32),
                gr.update(value=80, minimum=40, maximum=80, step=1),
            )
        elif edition == "v2":
            self.inference_config = OmegaConf.load(
                os.path.join(
                    self.config_dir, "easyanimate_video_magvit_motion_module_v2.yaml"
                )
            )
            return (
                gr.update(),
    ...
    def generate(
        ...
        if (
            ...
            if is_api:
                return (
                    "",
                    f"Please select an image to video pretrained model while using long video generation.",
                )
            else:
                raise gr.Error(
                    f"Please select an image to video pretrained model while using long video generation."
        ...
        if start_image is None and end_image is not None:
            if is_api:
                return (
                    "",
                    f"If specifying the ending image of the video, please specify a starting image of the video.",
                )
            else:
                raise gr.Error(
                    f"If specifying the ending image of the video, please specify a starting image of the video."
        ...
        if self.lora_model_path != "none":
            # lora part
            self.pipeline = merge_lora(
                self.pipeline, self.lora_model_path, multiplier=lora_alpha_slider
            )
                    ...
                    while init_frames < length_slider:
                        if last_frames >= length_slider:
                            if self.pipeline.vae.quant_conv.weight.ndim == 5:
                                mini_batch_encoder = (
                                    self.pipeline.vae.mini_batch_encoder
                                )
                                _partial_video_length = length_slider - init_frames
                                _partial_video_length = int(
                                    _partial_video_length
                                    // mini_batch_encoder
                                    * mini_batch_encoder
                                )
                            else:
                        ...
                        if init_frames != 0:
                            mix_ratio = (
                                torch.from_numpy(
                                    np.array(
                                        [
                                            float(_index) / float(overlap_video_length)
                                            for _index in range(overlap_video_length)
                                        ],
                                        np.float32,
                                    )
                                )
                            ...
                            new_sample = torch.cat(
                                [new_sample, sample[:, :, overlap_video_length:]], dim=2
                            )
                        ...
                        start_image = [
                            Image.fromarray(
                                (
                                    sample[0, :, _index].transpose(0, 1).transpose(1, 2)
                                    * 255
                                )
                                .numpy()
                                .astype(np.uint8)
                        ...
                        init_frames = (
                            init_frames + _partial_video_length - overlap_video_length
                        )
        ...
        if is_image or length_slider == 1:
            ...
            if is_api:
                ...
                if gradio_version_is_above_4:
                    return (
                        gr.Image(value=save_sample_path, visible=True),
                        gr.Video(value=None, visible=False),
                        "Success",
                    )
                else:
                    return (
                        gr.Image.update(value=save_sample_path, visible=True),
            ...
            save_videos_grid(
                sample, save_sample_path, fps=12 if self.edition == "v1" else 24
            )
            ...
            if is_api:
                ...
                if gradio_version_is_above_4:
                    return (
                        ...
                        gr.Video(value=save_sample_path, visible=True),
                        "Success",
                    )
                else:
                    return (
                        gr.Image.update(visible=False, value=None),
                        gr.Video.update(value=save_sample_path, visible=True),
                        "Success",
                    )
    ...
    with gr.Blocks(css=css) as demo:
        ...
        with gr.Column(variant="panel"):
            ...
            with gr.Row():
                ...
                diffusion_transformer_refresh_button.click(
                    fn=refresh_diffusion_transformer,
                    inputs=[],
                    outputs=[diffusion_transformer_dropdown],
                )
            ...
            with gr.Row():
                ...
                motion_module_refresh_button = gr.Button(
                    value="\U0001F503", elem_classes="toolbutton", visible=False
                )
        ...
        with gr.Column(variant="panel"):
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    base_resolution = gr.Radio(
                        label="Base Resolution of Pretrained Models",
                        value=512,
                        choices=[512, 768, 960],
                        visible=False,
                    )
                    ...
                    with gr.Group():
                        ...
                        with gr.Row():
                            length_slider = gr.Slider(
                                label="Animation length (视频帧数)",
                                value=144,
                                minimum=8,
                                maximum=144,
                                step=8,
                            )
                            overlap_video_length = gr.Slider(
                                label="Overlap length (视频续写的重叠帧数)",
                                value=4,
                                minimum=1,
                                maximum=4,
                                step=1,
                            ...
                            partial_video_length = gr.Slider(
                                label="Partial video generation length (每个部分的视频生成帧数)",
                                value=72,
                                minimum=8,
                                maximum=144,
                                step=8,
                    ...
                    with gr.Row():
                        ...
                        seed_button.click(
                            fn=lambda: (
                                gr.Textbox(value=random.randint(1, 1e8))
                                if gradio_version_is_above_4
                                else gr.Textbox.update(value=random.randint(1, 1e8))
                            ),
                            inputs=[],
                    ...
                    generate_button = gr.Button(
                        value="Generate (生成)", variant="primary"
                    )

                with gr.Column():
                    result_image = gr.Image(
                        label="Generated Image (生成图片)",
                        interactive=False,
                        visible=False,
                    )
                    result_video = gr.Video(
                        label="Generated Animation (生成视频)", interactive=False
                    )
            ...
            def upload_generation_method(generation_method):
                if generation_method == "Video Generation":
                    return [
                        gr.update(visible=True, maximum=144, value=144),
                        gr.update(visible=False),
                        gr.update(visible=False),
                    ]
                    ...
                    return [
                        gr.update(visible=True, maximum=1440),
                        gr.update(visible=True),
                        gr.update(visible=True),
                    ]
            ...
            resize_method.change(
                upload_resize_method,
                resize_method,
                [width_slider, height_slider, base_resolution],
            )
...
def ui_modelscope(
    edition, config_path, model_name, savedir_sample, low_gpu_memory_mode, weight_dtype
):
    controller = EasyAnimateController_Modelscope(
        edition,
        config_path,
        model_name,
        savedir_sample,
    ...
    with gr.Blocks(css=css) as demo:
                ...
                with gr.Column(visible=False):
                    ...
                    with gr.Row():
                        lora_model_dropdown = gr.Dropdown(
                            ...
                            choices=[
                                "none",
                                "easyanimatev2_minimalism_lora.safetensors",
                            ],
                            value="none",
                            interactive=True,
                        )
        ...
        with gr.Column(variant="panel"):
            ...
            def upload_generation_method(generation_method):
                if generation_method == "Video Generation":
                    return gr.update(
                        visible=True, minimum=8, maximum=48, value=48, interactive=True
                    )
                elif generation_method == "Image Generation":
...
def post_eas(
    ...
    response = session.post(
        url=f'{os.environ.get("EAS_URL")}/easyanimate/infer_forward',
        json=datas,
        timeout=300,
    )
...
def ui_eas(edition, config_path, model_name, savedir_sample):
    controller = EasyAnimateController_EAS(
        edition, config_path, model_name, savedir_sample
    )
        ...
        with gr.Column(variant="panel"):
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    if edition == "v1":
                        width_slider = gr.Slider(
                            label="Width", value=512, minimum=384, maximum=704, step=32
                        )
                        height_slider = gr.Slider(
                            label="Height", value=512, minimum=384, maximum=704, step=32
                        )
                        ...
                        cfg_scale_slider = gr.Slider(
                            label="CFG Scale", value=6.0, minimum=0, maximum=20
                        )
                    else:
                        resize_method = gr.Radio(
                            ["Generate by", "Resize to the Start Image"],
                        ...
                        with gr.Accordion("Image to Video", open=True):
                            ...
                            with gr.Accordion(
                                "The image at the ending of the video (Optional)",
                                open=False,
                            ):
                                end_image = gr.Image(
                                    label="The image at the ending of the video (Optional)",
                                    show_label=True,
                                    elem_id="i2v_end",
                                    sources="upload",
                                    type="filepath",
                ...
                with gr.Column():
                    result_image = gr.Image(
                        label="Generated Image", interactive=False, visible=False
                    )
                    result_video = gr.Video(
