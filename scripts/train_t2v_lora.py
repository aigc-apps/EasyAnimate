...
def log_validation(vae, text_encoder, tokenizer, transformer3d, network, config, args, accelerator, weight_dtype, global_step):
    # try:
        ...
        for i in range(len(args.validation_prompts)):
            if args.train_mode != "normal":
                image = Image.open(args.validation_images[i])
                input_video = torch.tile(
                    torch.from_numpy(np.array(image)).permute(2, 0, 1).unsqueeze(1).unsqueeze(0), 
                    [1, 1, 16, 1, 1]
                ) / 255
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="webvid",
        help=(
            'The format of training data. Support `"webvid"`'
            ' (default), `"normal"`.'
        ),
    )
    parser.add_argument(
