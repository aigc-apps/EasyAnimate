...
def log_validation(vae, text_encoder, tokenizer, transformer2d, args, accelerator, weight_dtype, global_step):
    try:
...
def main():
    ...
    for name, param in transformer2d.named_parameters():
        for trainable_module_name in args.trainable_modules + args.trainable_modules_low_learning_rate:
            if trainable_module_name in name:
                param.requires_grad = True
                break

    # Create EMA for the transformer2d.
    if args.use_ema:
        ema_transformer2d = Transformer2DModel.from_pretrained(
            args.pretrained_model_name_or_path, subfolder="transformer", 
        )
        ema_transformer2d = EMAModel(ema_transformer2d.parameters(), model_cls=Transformer2DModel, model_config=ema_transformer2d.config)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                if args.use_ema:
                    ema_transformer2d.save_pretrained(os.path.join(output_dir, "transformer_ema"))
        ...
        def load_model_hook(models, input_dir):
            if args.use_ema:
                load_model = EMAModel.from_pretrained(os.path.join(input_dir, "transformer_ema"), Transformer2DModel)
                ema_transformer2d.load_state_dict(load_model.state_dict())
                ema_transformer2d.to(accelerator.device)
                del load_model
    ...
    logging.info("Add transformer2d parameters")
    trainable_params = list(filter(lambda p: p.requires_grad, transformer2d.parameters()))
    ...
    transformer2d, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        transformer2d, optimizer, train_dataloader, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(transformer2d):
            ...
            if accelerator.sync_gradients:
                ...
                if args.use_ema:
                    ema_transformer2d.step(transformer2d.parameters())
                ...
                if accelerator.is_main_process:
                    if args.validation_prompts is not None and global_step % args.validation_steps == 0:
                        if args.use_ema:
                            ...
                            ema_transformer2d.store(transformer2d.parameters())
                            ema_transformer2d.copy_to(transformer2d.parameters())
                        log_validation(
                            vae,
                            text_encoder,
                            tokenizer,
                        ...
                        if args.use_ema:
                            # Switch back to the original transformer2d parameters.
                            ema_transformer2d.restore(transformer2d.parameters())
    ...
    if accelerator.is_main_process:
        transformer2d = unwrap_model(transformer2d)
        if args.use_ema:
            ema_transformer2d.copy_to(transformer2d.parameters())
