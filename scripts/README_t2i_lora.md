## EN、Training text to image Lora model
...
Then, set scripts/train_t2i_lora.sh.
```
...
Then, we run scripts/train_t2i_lora.sh.
```sh
sh scripts/train_t2i_lora.sh
```
...
然后，进入scripts/train_t2i_lora.sh进行设置。
```
export DATASET_NAME="datasets/diffusers_datasets/"
